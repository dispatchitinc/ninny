...
RSpec.describe Ninny::Commands::NewStaging do
  ...
  context '#delete_old_branches' do
    ...
    context 'with old branches' do
      let(:branch_1) { double(:branch_1) }
      let(:branch_2) { double(:branch_2) }
      before { allow(subject).to receive(:extra_branches).and_return([branch_1, branch_2]) }

      it 'should delete old branches if flag is set' do
        expect(Ninny.git).to receive(:delete_branch).with(branch_1)
        expect(Ninny.git).to receive(:delete_branch).with(branch_2)
      ...
      it 'should prompt for deletion if flag is not set' do
        ...
        expect(Ninny.git).to receive(:delete_branch).with(branch_1)
        expect(Ninny.git).to receive(:delete_branch).with(branch_2)
        expect_any_instance_of(TTY::Prompt).to receive(:yes?).with(/Do you want to delete the old staging branch(es)?/).and_return(true)
        subject.delete_old_branches
      end
    end
  end
  ...
  context '#extra_branches' do
      let(:branch_1) { double(:branch_1, name: 'staging.2019.09.16') }
      let(:branch_2) { double(:branch_2, name: 'staging.2019.09.15') }
      before { expect(Ninny.git).to receive(:branches_for).with('staging').and_return([branch_1, branch_2]) }
    it 'should fetch extra branches from git' do
      expect(subject.extra_branches).to eq([branch_2])
    end
  end
end
